...
runModel <- function(sampleID, outType="dTabs", uncRCP=0,
  ...
  if(procInSample){
    ...
    # If there are no rows where N=0 then return -all_rows
    if(length(x0) == 0) {
      x0 <- -c(1:nrow(sampleX))
    }
    sampleX <- sampleX[-x0]
...
create_prebas_input.f = function(r_no, clim, data.sample, nYears,
  ...
  init_save_path <- "scratch/project_2000994/PREBASruns/finRuns/Rsrc/samuel/llindfors/initMultiSiteParams.rdata"
